...
export class CarsService {
  ...
  async findAll(queryCarDto: QueryCarDto) {
    ...
    if (color) {
      query.andWhere('cars.color = :color', { color });
    }

    if (minimalPrice && maximalPrice) {
      query.andWhere(
        'cars.price >= :minimalPrice AND cars.price <= :maximalPrice',
        {
          minimalPrice,
          maximalPrice,
        },
      );
    } else if (minimalPrice) {
      query.andWhere('cars.price >= :minimalPrice', {
        minimalPrice,
      });
    } else if (maximalPrice) {
      query.andWhere('cars.price <= :maximalPrice', {
        maximalPrice,
      });
    }
